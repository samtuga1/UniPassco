...
sealed class AuthenticationState<T> with _$AuthenticationState<T> {
  ...
  const factory AuthenticationState.verifyTokenSuccess() =
      VerifyTokenSuccess<T>;
  const factory AuthenticationState.logingIn() = LogingIn<T>;
  const factory AuthenticationState.loginSuccess({required User user}) =
      LoginSuccess<T>;
  const factory AuthenticationState.requestingPasswordReset() =
      RequestingPasswordReset<T>;
  const factory AuthenticationState.requestPasswordResetSuccess() =
      RequestPasswordResetSuccess<T>;
  const factory AuthenticationState.resettingPassword() = ResettingPassword<T>;
  const factory AuthenticationState.resettingPasswordSuccess() =
      ResettingPasswordSuccess<T>;
